...
from .lkio import parse_filetype
...
class TargetPixelFile(DataSet):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif isinstance(path, fits.HDUList):
            hdu = path
        else:
            warnings.warn("Must provide a file path or hdulist")

        for ii in range(len(hdu)):
            if ii == 0:
                self.primary_header = hdu[0].header
            else:
                setattr(self, f"hdu{ii}_header", hdu[ii].header)

        self.data_cubes = parse_filetype(hdu)
    ...
    def bin(self, binsize: Union[int, tuple] = 1):
        """
        ...
        # binned_object = deepcopy(self)
        # binned_object.data_cubes = data_cubes.downsample(binsize)
        # TODO: adjust wcs info here? Anything else?
        # return binned_object
        ...
    ...
    def spatial_bin(self, binsize: Union[int, tuple] = 1):
        ...
        """
        # AttributeError: 'DataSet' object has no attribute 'spatial_downsample'
        # return self.data_cubes.downsample(binsize)
        ...

    def __repr__(self):
        # TODO: not sure if all HLSPs have the OBJECT keyword?
        return f"TargetPixelFile Object (ID: {self.get_keyword('OBJECT')})"
    ...
    @property
    def len(self):
        return self.data_cubes["FLUX"].shape[0]
    ...
    def pos_corr1(self):
        ...
        return getattr(self.data_cubes["FLUX"], "POS_CORR1")  # [self.quality_mask]

    @property
    def pos_corr2(self):
        """Returns the row position correction."""
        return getattr(self.data_cubes["FLUX"], "POS_CORR2")  # [self.quality_mask]
    ...
    def shape(self):
        ...
        return self.data_cubes["FLUX"].shape

    @property
    def time(self) -> Time:
        """Returns the time for all good-quality cadences."""
        time_values = getattr(self.data_cubes["FLUX"], "TIME")  # [self.quality_mask]
    ...
    def cadenceno(self):
        ...
        # TODO: Add try block here? Do all HLSPs have this property?
        cadenceno = getattr(self.data_cubes["FLUX"], "CADENCENO")  # [self.quality_mask]
    ...
    def flux(self) -> Quantity:
        ...
        if self.get_header(ext=1).get("TUNIT5") == "e-/s":
            unit = "electron/s"
        return Quantity(self.data_cubes["FLUX"], unit=unit)
        # return Quantity(self.data_cubes['FLUX'][self.quality_mask], unit=unit)

    @property
    def flux_err(self) -> Quantity:
        """Returns the flux uncertainty for all good-quality cadences."""
        unit = None
        # TODO: in general I don't like how units are handled. What would be a better way?
        if self.get_header(ext=1).get("TUNIT6") == "e-/s":
            unit = "electron/s"
        # return Quantity(self.data_cubes['FLUX_ERR'][self.quality_mask], unit=unit)
        return Quantity(self.data_cubes["FLUX_ERR"], unit=unit)
